...
The second problem is the [lack of any guarantee](http://stackoverflow.com/questions/14387104/atomic-writes-in-linux) that all the changes made by a single process will be performed in a single `write` call, as the OS could interrupt an ongoing `write` call halfway through and expect you to schedule another `write` call to write the remaining data. This means that multiple processes writing to a single file could have their `write` calls interleaved, causing the new contents of the file to be out of order.
