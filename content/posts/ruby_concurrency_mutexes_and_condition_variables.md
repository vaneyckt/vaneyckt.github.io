...
It should hopefully be clear that none of this would have happened if we had made sure that `counters.map! { |counter| counter + 1 }` was atomic. This would have made it impossible for the second thread to just come in and modify the intermediate state of the object being modified by the first thread.
